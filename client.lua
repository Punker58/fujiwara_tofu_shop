...
local blips = {
	{title="tofu", colour=4, id=488, x = 355.16174316406, y = 331.60290527344, z = 116.34196472168},  
}
	...
	while true do
		...
		if isInJobPizz == false then
			...
			if GetDistanceBetweenCoords(tofu.x, tofu.y, tofu.z, GetEntityCoords(GetPlayerPed(-1),true)) < 1.5 then
				drawTxt("PREMI E PER INIZIARE A CONSEGNARE IL TOFU",2, 1, 0.45, 0.03, 0.80,255,255,51,255)
				if IsControlJustPressed(1,38) then
					isInJobPizz = true
					isToHouse = true
					sigcasa = math.random(1, 13)
					-- [INFO] TriggerEvent('chatMessage', 'BUNTA FUJIWARA', {255, 0, 0}, sigcasa)
					px = casas[sigcasa].x
					py = casas[sigcasa].y
					pz = casas[sigcasa].z
					distancia = round(GetDistanceBetweenCoords(tofu.x, tofu.y, tofu.z, px,py,pz))
					paga = distancia * multiplicador_De_dinero
					-- [INFO] TriggerEvent('chatMessage', 'BUNTA FUJIWARA', {255, 0, 0}, distancia)
					spawn_ae86()
		...
		if isToHouse == true then
			...
			drawTxt("USA L'AE86 DI BUNTA FUJIWARA PER ARRIVARE  "..destinol .." E CONSEGNARE IL TOFU",4, 1, 0.45, 0.92, 0.70,255,255,255,255)
		...
		if isToPizzaria == true then
			drawTxt("RITORNA AL FUJIWARA TOFU SHOP PER RICEVERE LA TUA PAGA",4, 1, 0.45, 0.92, 0.70,255,255,255,255)
			DrawMarker(1,spawnae86.x,spawnae86.y,spawnae86.z, 0, 0, 0, 0, 0, 0, 2.5001, 2.5001, 0.6001,255,255,51, 200, 0, 0, 0, 0)  
				if GetDistanceBetweenCoords(spawnae86.x,spawnae86.y,spawnae86.z, GetEntityCoords(GetPlayerPed(-1),true)) < 3 then
					drawTxt("PREMI E PER CONSEGNARE IL VEICOLO E RICEVERE LA TUA PAGA",2, 1, 0.45, 0.03, 0.80,255,255,51,255)
					if IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("ae86"))  then
						if IsControlJustPressed(1,38) then
							if IsInVehicle() then
								TriggerEvent('chatMessage', 'BUNTA FUJIWARA', {255, 0, 0},"OTTIMO LAVORO, QUESTA E' LA TUA PAGA. HAI RICEVUTO "..paga.."$")
								...
								TriggerEvent('chatMessage', 'BUNTA FUJIWARA', {255, 0, 0},"PERCASO HAI FATTO CADERE IL BICCHIERE D'ACQUA?")
							end
						end
					else
						TriggerEvent('chatMessage', 'BUNTA FUJIWARA', {255, 0, 0},"PERCASO HAI FATTO CADERE IL BICCHIERE D'ACQUA?")
